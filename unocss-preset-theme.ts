...
function generateCSSVariables(obj: DeepRecord, prefix: string[] = []): string[] {
  return Object.entries(obj).flatMap(([key, value]) => {
    const currentPath = [...prefix, key]
    if (typeof value === 'string') {
      return [`--${currentPath.join('-')}: ${value}`]
    }
    return generateCSSVariables(value as DeepRecord, currentPath)
  })
}
...
const light = {
  colors: {
    ...
    destructive: { DEFAULT: 'rgb(199, 62, 58)', foreground: 'rgb(255, 255, 255)' },
