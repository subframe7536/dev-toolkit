  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        class={cls(
          'fixed left-50% top-50% z-50 grid w-90% max-w-xl translate--50% gap-4 border bg-background text-foreground p-4 shadow-lg duration-200 data-[expanded]:(animate-in fade-in-0 zoom-in-95 slide-in-b-2 duration-200) data-[closed]:(animate-out fade-out-0 zoom-out-95 slide-out-b-2 duration-200) md:w-full rounded-lg',
          local.class,
        )}
        {...rest}
      >
        {local.children}
        <DialogPrimitive.CloseButton class="rounded-sm bg-inherit opacity-70 ring-offset-background transition transition-opacity right-4 top-4 absolute focus:effect-fv disabled:effect-dis hover:opacity-100">
          <Icon name="lucide:x" />
